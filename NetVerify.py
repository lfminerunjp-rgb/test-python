...
def trace_check(ip):
    ...
    try:
        ...
        return "\n".join([l for l in res.stdout.splitlines() if l.strip() and not any(x in l for x in ["トレース", "ホップ"])])
    except: return "Traceroute失敗"

def find_teraterm():
    for p in [r"C:\Program Files (x86)\teraterm\ttpmacro.exe", r"C:\Program Files\teraterm\ttpmacro.exe"]:
    ...
    if EXCEL_AVAILABLE and os.path.exists(xlsx_path):
        try:
            ...
            for row in ws.iter_rows(min_row=2, values_only=True):
                ...
                h['command_list'] = [c.strip() for c in str(row[7] if len(row) > 7 else "").split('\n') if c.strip()]
                hosts.append(h)
            return hosts
        except: pass
    return []
...
def main():
    # --- 修正の核心：cisco_s200のクラス定義を直接書き換える ---
    # ConnectHandlerに不正な引数を渡すことなく、内部の正規表現だけを書き換えます。
    if 'cisco_s200' in CLASS_MAPPER:
        # C1200が返す 'User Name:' にマッチするように正規表現を修正
        # これによりConnectHandler内部の処理が正常にパターンを検出できるようになります。
        CLASS_MAPPER['cisco_s200'].auth_username_pattern = r'User[ \t]*Name[:]'
        CLASS_MAPPER['cisco_s200'].auth_password_pattern = r'Password[:]'
            ...
            while True:
                ...
                if mode_in in ['0', '0t']:
                    for idx in indices:
                        ...
                        status = f"{GREEN}[SUCCESS]{RESET}" if ping_check(ip) else f"{RED}[FAIL]{RESET}"
                        print(f"  \n{status} {host.get('name')} ({ip})")
                        if mode_in == '0t': print(f"    [Trace Result]\n{trace_check(ip)}\n\n")
                    continue

                if mode_in == '1':
                    tt_macro_exe = tt_macro_exe or find_teraterm()
                    if not tt_macro_exe: print(f"{RED}[!] ttpmacro.exeが見つかりません。{RESET}")
                    else:
                        for idx in indices:
                            host = hosts[idx]; ttl = create_ttl_macro(host)
                            subprocess.Popen([tt_macro_exe, ttl]); time.sleep(0.5)
                            try: os.remove(ttl)
                            except: pass
                    continue
                ...
                for i, idx in enumerate(indices):
                    ...
                    device_type = str(host.get('vendor', 'cisco_ios')).lower()
                    if str(host.get('protocol')).lower() == 'telnet': device_type += '_telnet'

                    # Netmikoが公式にサポートしている引数のみを使用
                    device = { 
                    ...
                    try:
                        with ConnectHandler(**device) as net:
                                ...
                                for kw in search_keywords:
                                    for line in raw_out.splitlines():
                                        if kw.lower() in line.lower():
                                            search_hits[kw].append(f"[{cmd}] " + re.sub(re.escape(kw), lambda m: f"{YELLOW}{m.group()}{RESET}", line.strip(), flags=re.IGNORECASE))
                                if mode_in in ['3', '4']:
                                    try:
                            ...
                            if search_hits:
                                print(f"\n{YELLOW}[ 検索結果 ]{RESET}"); [print(f"▼ '{k}':\n" + "\n".join(v)) for k, v in search_hits.items()]
                            if mode_in in ['2', '4']:
                                with open(os.path.join(LOG_DIR, f"{h_file}_{datetime.now().strftime('%Y%m%d')}.log"), "a", encoding='utf-8') as f: f.write(log_body)
                                    ...
                                    if diff:
                                        ...
                                        for cat, det in diff.items():
                                            ...
                                            for path, val in items:
                                                c, sec = safe_parse_path(path)
                                                print(f"  [{c}] {sec}: {RED if 'removed' in cat else BLUE}{cat}{RESET}")
                                    else: print(f"  {GREEN}[OK] 差分なし{RESET}")
                                if os.path.exists(snap_p): os.rename(snap_p, os.path.join(SNAPSHOT_DIR, f"snapshot_{h_file}_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"))
