...
def trace_check(ip):
    """OSのTracerouteコマンドを使用して経路確認 (モード0t用) -d -h 15 -w 1000"""
    ...
    try:
        ...
        for line in lines:
            l_s = line.strip()
            if not l_s: continue
            if any(x in l_s for x in ["へのルートをトレースしています", "経由するホップ数は最大", "トレースを完了しました"]): continue
            filtered.append(line)
        return "\n".join(filtered)
    except KeyboardInterrupt:
        return f"{YELLOW}Tracerouteは中断されました。{RESET}"
    except Exception as e:
        return f"Traceroute失敗: {e}"

def find_teraterm():
    standard_paths = [
        r"C:\Program Files (x86)\teraterm\ttpmacro.exe",
        r"C:\Program Files\teraterm\ttpmacro.exe"
    ]
...
def create_ttl_macro(host_info):
    ...
    with open(macro_path, "w", encoding='cp932') as f:
        ...
        # 接続直後のプロンプト判定 (C1200等のUser Name:要求に対応)
        f.write("\n; 接続直後の入力要求チェック\n")
...
def main():
    ...
    while True:
        ...
        while True:
            ...
            # 複製防止のため、対象一覧を一度だけ出力する
            print(f"\n{YELLOW}[ 対象一覧 - モード: {mode_map[mode_in]} ]{RESET}")
            for i, h in enumerate(hosts): 
                print(f"{i}: {h.get('name')} ({h.get('ip')}) - [{len(h.get('command_list', []))} cmds]")
            ...
            try:
                ...
                for i, idx in enumerate(indices):
                    ...
                    device = { 
                        ...
                        'auth_timeout': 20 # 2段階認証用に少し長めに待機
                    }

                    print("\n\n\n\n\n" + "=" * 70); print(f"{GREEN}>>> [{h_name}]{RESET}")
                    try:
                        with ConnectHandler(**device) as net:
                            # 接続直後、もし一般モード(>)ならenableを実行
                            # C1200等でSSHログイン後さらにログインが必要な場合、Netmikoは通常ここで自動的にユーザー情報を再送しようと試みます
                            ...
                            if search_hits:
                                print(f"\n{YELLOW}[ 検索結果 ]{RESET}")
                                for kw, lines in search_hits.items():
                                    print(f"▼ '{YELLOW}{kw}{RESET}':"); [print(f"  {l}") for l in lines]
                                    ...
                                    if diff:
                                        ...
                                        for cat, det in diff.items():
                                            ...
                                            for path, val in items:
                                                ...
                                                display_sec = sec.replace('_via_', ' via ')
                                                if cat == 'values_changed':
                                                    report[c].append(f"  対象: {display_sec} | {RED}{val['old_value']}{RESET} -> {BLUE}{val['new_value']}{RESET}")
                                                elif 'iterable_item_added' in cat:
                                                    report[c].append(f"  対象: {display_sec} | {BLUE}+ ADDED: {val}{RESET}")
                                                elif 'iterable_item_removed' in cat:
                                                    report[c].append(f"  対象: {display_sec} | {RED}- REMOVED: {val}{RESET}")
                                                elif 'dictionary_item_added' in cat:
                                                    report[c].append(f"  対象: {display_sec} {BLUE}+ ADDED : {format_val(val)}{RESET}")
                                                elif 'dictionary_item_removed' in cat:
                                                    report[c].append(f"  対象: {display_sec} {RED}- REMOVED {RESET}")
                                        for cn, msgs in report.items():
                                            print(f"\n{YELLOW}[差分あり] {cn}{RESET}"); [print(m) for m in msgs]
                                    else: print(f"  {GREEN}[OK] 差分なし{RESET}")
                                else: print(f"  {BLUE}[NEW] スナップショットを新規作成しました。{RESET}")
                                
                                # スナップショットのリネーム履歴保存
                                if os.path.exists(snap_p):
                                    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
                                    archive_p = os.path.join(SNAPSHOT_DIR, f"snapshot_{h_file}_{ts}.json")
